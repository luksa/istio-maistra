...
func (r *RealDependencies) execute(cmd string, ignoreErrors bool, input io.Reader, args ...string) error {
	if r.CNIMode {
		originalCmd := cmd
...
func (r *RealDependencies) executeXTables(cmd string, ignoreErrors bool, input io.Reader, args ...string) error {
	if r.CNIMode {
		originalCmd := cmd
		cmd = constants.NSENTER
		args = append([]string{fmt.Sprintf("--net=%v", r.NetworkNamespace), "--", originalCmd}, args...)
...
func (r *RealDependencies) RunOrFail(cmd string, stdin io.Reader, args ...string) {
	var err error
...
// Run runs a command
func (r *RealDependencies) Run(cmd string, stdin io.Reader, args ...string) (err error) {
	if XTablesCmds.Contains(cmd) {
		err = r.executeXTables(cmd, false, stdin, args...)
	} else {
		err = r.execute(cmd, false, stdin, args...)
...
func (r *RealDependencies) RunQuietlyAndIgnore(cmd string, stdin io.Reader, args ...string) {
	if XTablesCmds.Contains(cmd) {
